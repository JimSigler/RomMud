...
void do_steal( CHAR_DATA *ch, char *argument )
{
    ...
    if(get_curr_stat(ch, STAT_LUC) > 13)
	percent -= 20;
...
void do_buy( CHAR_DATA *ch, char *argument )
{
	...
	roll = number_percent();
	if (!IS_OBJ_STAT(obj,ITEM_SELL_EXTRACT) 
	&& roll < get_skill(ch,gsn_haggle) + get_curr_stat(ch, STAT_CHA) )
	{
...
void do_sell( CHAR_DATA *ch, char *argument )
{
    ...
    /* haggle */
    roll = number_percent();
    if (!IS_OBJ_STAT(obj,ITEM_SELL_EXTRACT) && roll < (get_skill(ch,gsn_haggle) + get_curr_stat(ch, STAT_CHA)) )
    {
