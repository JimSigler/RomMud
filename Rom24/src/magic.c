...
void spell_dragonborn_fire_breath( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
	...
	if (vch == victim) /* full damage */
	{
		...
		sprintf(buf, "dragon breath: direct victim SAVED  damage: %d", dam/2);
		do_immmessage(ch,buf);
	    }
	    else
	    {
		fire_effect(vch,level,dam,TARGET_CHAR);
		damage(ch,vch,dam,sn,DAM_FIRE,TRUE);
		sprintf(buf, "dragon breath: direct victim FULL  damage: %d", dam);
		do_immmessage(ch, buf);
	    }
	}
	else /* partial damage */
	{
		...
		sprintf(buf, "dragon breath: not victim SAVED  damage: %d", dam/4);
		bug(buf, 0);
	    }
	    else
	    {
		fire_effect(vch,level/2,dam/4,TARGET_CHAR);
		damage(ch,vch,dam/2,sn,DAM_FIRE,TRUE);
		sprintf(buf, "dragon breath: not victim full  damage: %d", dam/2);
		bug(buf, 0);
	    }
	}
    }
