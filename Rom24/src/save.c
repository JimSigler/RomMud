...
void fread_char( CHAR_DATA *ch, FILE *fp )
{
	...
	case 'E':
	    ...
	    KEY( "Etrait",        ch->etrait,               fread_number( fp ) );
	    break;
	case 'F':
	    KEY( "Fheight",      ch->pcdata->fheight,    fread_number( fp ) );
	    break;
	case 'G':
	...
	case 'H':
	    ...
	    KEY( "Htrait",      ch->htrait,             fread_number( fp ) );
	...
	case 'I':
	    ...
	    KEY( "Iheight",     ch->pcdata->iheight,    fread_number( fp ) );
	    break;
    ...
    for ( ; ; )
    {
		...
		paf->where	= fread_number(fp);
    	    ...
    	    if (!str_cmp(word,"AMod"))
    	    {
    	     	...
    	     	for (stat = 0; stat < MAX_STATS-2; stat++)
