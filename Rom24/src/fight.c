...
bool check_parry( CHAR_DATA *ch, CHAR_DATA *victim )
{
	...
	if (IS_NPC(victim))
     ...
     // add some luck to parry
    if(!IS_NPC(victim))
    {
	int modifier;
	modifier = get_curr_stat(victim, STAT_LUC);
	if(modifier > 11)
	    chance += modifier / 2;
	else
	    chance -= modifier;
...
void do_backstab( CHAR_DATA *ch, char *argument )
{
    ...
    if ( number_percent( ) < (get_skill(ch,gsn_backstab) + get_curr_stat(ch, STAT_LUC))
...
void do_rescue( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_NPC(ch) && IS_NPC(victim) && ch != victim->master)
    {
