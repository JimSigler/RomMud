...
int etrait_lookup (const char *ecolor)
 {
    int etrait;

    for ( etrait = 0; etrait_table[etrait].ecolor != NULL; etrait++)
    {
         if (LOWER(ecolor[0]) == LOWER(etrait_table[etrait].ecolor[0])
         &&  !str_prefix( ecolor,etrait_table[etrait].ecolor))
             return etrait;
    }
...
int htrait_lookup (const char *hcolor)
 {
    int htrait;

    for ( htrait = 0; htrait_table[htrait].hcolor != NULL; htrait++)
    {
         if (LOWER(hcolor[0]) == LOWER(htrait_table[htrait].hcolor[0])
         &&  !str_prefix( hcolor,htrait_table[htrait].hcolor))
             return htrait;
    }
