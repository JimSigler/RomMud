...
char *format_obj_to_char( OBJ_DATA *obj, CHAR_DATA *ch, bool fShort )
{
    ...
    if ( strlen(buf) <= 0)
    {
	strcat(buf, "This object has no description, please tell an IMP.");
    }
...
void show_char_to_char_1( CHAR_DATA *victim, CHAR_DATA *ch )
{
    ...
    {
	sprintf(buf, "\n\rBefore you is a");
	if(victim->pcdata->fheight > 0)
	{
	    sprintf(buf2, " %d'%d\" tall", victim->pcdata->fheight, victim->pcdata->iheight);
	    strcat(buf, buf2);
	}
	sprintf(buf2, " %s",race_table[victim->race].name);
	strcat(buf, buf2);
	if(victim->etrait && victim->htrait)
	{
	    sprintf(buf2, " with %s eyes and %s hair", etrait_table[victim->etrait].ecolor, htrait_table[victim->htrait].hcolor);
	    strcat(buf, buf2);
	}
	strcat(buf, ".");
...
void draw_underline( CHAR_DATA *ch, bool top )
{
    char buf[MAX_STRING_LENGTH];
    int max_screen_size = 80;
    int count;
    
    sprintf(buf, " ");
    for(count = 0; count < max_screen_size; count++)
    {
	if(count == 0 && !top)
	{
	    sprintf( buf, "|" );
	}
	else if( count == max_screen_size -1 )
	{
	    if(!top)
	    {
		strcat(buf, "|");
...
void send_formatted(char *message, CHAR_DATA *ch, int position)
{
    char buf[MAX_STRING_LENGTH];
    int max_screen_size = 80;
    int size_of_message = strlen_exclude_colors(message);
    int left_side;
    int right_side;
    int i;
    ...
    if(position == CENTER)
    {
	left_side = ( max_screen_size / 2 ) - ( size_of_message / 2 )-1;
	right_side = max_screen_size - left_side - size_of_message -1;
	
	for (i=0; i < left_side-1; i++)
	{
	    strcat(buf, " ");
	}
	strcat(buf, message);

	for (i=0; i< right_side-1; i ++)
	{
	    strcat(buf, " ");
	}
    }
    ...
    {
	left_side = max_screen_size - size_of_message - 2;
	for (i=0; i < left_side-1; i++)
	{
	    strcat(buf, " ");
	}
    ...
    {
	right_side = max_screen_size - size_of_message - 2;
	strcat(buf, message);
	for (i=0; i < right_side -1; i++)
	{
	    strcat(buf, " ");
...
void do_score( CHAR_DATA *ch, char *argument )
{
    ...
    draw_underline( ch, TRUE );
    
    sprintf( buf,
	"{G**{x{W %s%s {x{G**{x",
	ch->name,
	IS_NPC(ch) ? "" : ch->pcdata->title);
    send_formatted( buf, ch, CENTER );
    
    draw_underline( ch, FALSE );
    
    send_formatted( " ", ch, LEFT );
    
    sprintf( buf,
	"Level %d, %d years old (%d hours).",
	ch->level, get_age(ch),
        ( ch->played + (int) (current_time - ch->logon) ) / 3600);
    send_formatted( buf, ch, LEFT );
    ...
    sprintf(buf, "Height:%d'%d\"  Eye Color: %s  Hair Color: %s",
	ch->pcdata->fheight, ch->pcdata->iheight,
	etrait_table[ch->etrait].ecolor, htrait_table[ch->htrait].hcolor);
    send_formatted(buf, ch, LEFT);
	...
	send_formatted( "You are drunk.",   ch, LEFT );
    if ( !IS_NPC(ch) && ch->pcdata->condition[COND_THIRST] ==  0 )
	send_formatted( "You are thirsty.", ch, LEFT );
    if ( !IS_NPC(ch) && ch->pcdata->condition[COND_HUNGER]   ==  0 )
	send_formatted( "You are hungry.",  ch, LEFT );

    switch ( ch->position )
    {
    case POS_DEAD:     
	send_formatted( "You are DEAD!!", ch, LEFT );
	break;
    case POS_MORTAL:
	send_formatted( "You are mortally wounded.", ch, LEFT );
	break;
    case POS_INCAP:
	send_formatted( "You are incapacitated.",	ch, LEFT );
	break;
    case POS_STUNNED:
	send_formatted( "You are stunned.", ch, LEFT );
	break;
    case POS_SLEEPING:
	send_formatted( "You are sleeping.", ch, LEFT );
	break;
    case POS_RESTING:
	send_formatted( "You are resting.", ch, LEFT );
	break;
    case POS_SITTING:
	send_formatted( "You are sitting.", ch, LEFT );
	break;
    case POS_STANDING:
	send_formatted( "You are standing.", ch, LEFT );
	break;
    case POS_FIGHTING:
	send_formatted( "You are fighting.", ch, LEFT );
	break;
    }
	...
	if      (GET_AC(ch,i) >=  101 ) 
	{
	    sprintf(buf2,"hopelessly vulnerable to %s.", temp);
	    strcat(buf, buf2);
	}
	else if (GET_AC(ch,i) >= 80) 
	{
	    sprintf(buf2,"defenseless against %s.", temp);
	    strcat(buf, buf2);
	}
	else if (GET_AC(ch,i) >= 60)
	{
	    sprintf(buf2,"barely protected from %s.", temp);
	    strcat(buf, buf2);
	}
	else if (GET_AC(ch,i) >= 40)
	{
	    sprintf(buf2,"slightly armored against %s.", temp);
	    strcat(buf, buf2);
	}
	else if (GET_AC(ch,i) >= 20)
	{
	    sprintf(buf2,"somewhat armoed against %s.", temp);
	    strcat(buf, buf2);
	}
	else if (GET_AC(ch,i) >= 0)
	{
	    sprintf(buf2,"armored against %s.", temp);
	    strcat(buf, buf2);
	}
	else if (GET_AC(ch,i) >= -20)
	{
	    sprintf(buf2,"well armored against %s.", temp);
	    strcat(buf, buf2);
	}
	else if (GET_AC(ch,i) >= -40)
	{
	    sprintf(buf2,"very well armored against %s.", temp);
	    strcat(buf, buf2);
	}
	else if (GET_AC(ch,i) >= -60)
	{
	    sprintf(buf2,"heavily armored against to %s.", temp);
	    strcat(buf, buf2);
	}
	else if (GET_AC(ch,i) >= -80)
	{
	    sprintf(buf2,"superbly armored against %s.", temp);
	    strcat(buf, buf2);
	}
	else if (GET_AC(ch,i) >= -100)
	{
	    sprintf(buf2,"almost invulnerable to %s.", temp);
	    strcat(buf, buf2);
	}
	else
	{
	    sprintf(buf2,"divinely armored against %s.", temp);
	    strcat(buf, buf2);
	}
	...
	sprintf( buf, "You are " );
         ...
         if ( ch->alignment >  900 ) strcat( buf, "angelic." );
    else if ( ch->alignment >  700 ) strcat( buf, "saintly." );
    else if ( ch->alignment >  350 ) strcat( buf, "good."    );
    else if ( ch->alignment >  100 ) strcat( buf, "kind."    );
    else if ( ch->alignment > -100 ) strcat( buf, "neutral." );
    else if ( ch->alignment > -350 ) strcat( buf, "mean."    );
    else if ( ch->alignment > -700 ) strcat( buf, "evil."    );
    else if ( ch->alignment > -900 ) strcat( buf, "demonic." );
    else                             strcat( buf, "satanic." );
