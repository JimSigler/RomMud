...
/*
 * Calculate the hp, mana, and move gained based on luck
 * standard multiplier is 1
 * the divider is changed if they're lucky
 */
    int stat = get_curr_stat(ch, STAT_LUC) * 1.5;
    int perc = number_percent();
    int luck_multiplier = 1;

    sprintf(buf, "luck was %d and perc was %d.", stat, perc);
    bug(buf, 0);
    if(perc < stat)
    {
	sprintf(buf, "Luck is on your side.\n\r");
	send_to_char(buf, ch);
	luck_multiplier = 2;
    }
    ...
    // apply the luck multiplier
    add_hp *= luck_multiplier;
    add_mana *= luck_multiplier;
    add_move *= luck_multiplier;

    sprintf(buf, "HP gain: %d  MANA gain: %d  MOVE gain: %d", add_hp, add_mana, add_move);
    bug(buf, 0);
