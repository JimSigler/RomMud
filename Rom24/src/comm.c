...
int etrait_lookup args((const char *ecolor));
int htrait_lookup args((const char *hcolor));
...
void close_socket( DESCRIPTOR_DATA *dclose )
{
	...
	log_string( log_buf );
	/* if character has a pet, remove it as well */
...
void nanny( DESCRIPTOR_DATA *d, char *argument )
{
    ...
    bool fOld;
    int etrait,htrait,fheight,iheight,weight;
...
case CON_GET_ALIGNMENT:
	...
	write_to_buffer(d,"Please select your characters eye color:\n\r  ",0);
	write_to_buffer(d, "\n\r",0);
	write_to_buffer(d, "[ red blue green black brown purple grey white hazel ]\n\r",0);
        write_to_buffer(d,"\n\r",0);
        write_to_buffer(d,"What is your characters eye color? ",0);
        d->connected = CON_GET_ETRAIT;
        break;
...
case CON_GET_ETRAIT:
        ...
        etrait = etrait_lookup(argument);

        if ( etrait == 0 )
        {
            write_to_buffer(d,"That is not a valid eye color.\n\r",0);
            write_to_buffer(d,"The following eye colors are available:\n\r",0);
            write_to_buffer(d,"\n\r",0);
	    write_to_buffer(d,"[ red blue green black brown purple grey white hazel ]\n\r",0);
            write_to_buffer(d,
                "What is your characters eye color?",0);
            break;
         }
	...
	write_to_buffer(d,"Please select your characters hair color:\n\r  ",0);
	write_to_buffer(d, "\n\r", 0);
	write_to_buffer(d, "[ red blue green black brown purple grey white orange silver yellow ]\n\r", 0);
        write_to_buffer(d,"\n\r",0);
        write_to_buffer(d,"What is your characters hair color? ",0);
        d->connected = CON_GET_HTRAIT;
        break;
...
case CON_GET_HTRAIT:
        ...
        htrait = htrait_lookup(argument);

        if ( htrait == 0 )
        {
            write_to_buffer(d,"That is not a valid hair color.\n\r",0);
            write_to_buffer(d,"The following hair colors are available:\n\r",0);
            write_to_buffer(d,"\n\r",0);
	    write_to_buffer(d,"[ red blue green black brown purple grey white orange silver yellow ]\n\r", 0);
            write_to_buffer(d,"What is your characters hair color?",0);
            break;
        ...
        write_to_buffer(d,"\n\r",0);
        write_to_buffer(d,"What is your characters height in feet? ",0);
        d->connected = CON_GET_FHEIGHT;
        break;

case CON_GET_FHEIGHT:

        fheight = atoi(argument);


        if ( (fheight = atoi(argument)) <= 0 || fheight > 25 )
        {
             write_to_buffer(d,"Invalid height in feet, choose a reasonable value.\n\r",0);
             write_to_buffer(d,"\n\r",0);
             write_to_buffer(d,"Height is limited from 1 to 25.\n\r",0);
             break;

        }

        ch->pcdata->fheight = fheight;

        write_to_buffer(d,"\n\r",0);
        write_to_buffer(d,"What is your characters height in inches? ",0);
        d->connected = CON_GET_IHEIGHT;
        break;

case CON_GET_IHEIGHT:

        iheight = atoi(argument);


        if ( (iheight = atoi(argument)) <= 0 || iheight > 11 )
        {
             write_to_buffer(d,"Invalid height in inches, choose a reasonable value.\n\r",0);
             write_to_buffer(d,"\n\r",0);
             write_to_buffer(d,"Height in inches is limited from 1 to 11.\n\r",0);
             break;

        }

        ch->pcdata->iheight = iheight;

        write_to_buffer(d,"\n\r",0);
        write_to_buffer(d,"What is your characters weight? ",0);
...
case CON_GET_WEIGHT:
        ...
        if ( (weight = atoi(argument)) <= 50 || weight > 1000 )
        {
             write_to_buffer(d,"Invalid weight, choose a reasonable weight.\n\r",0);
             write_to_buffer(d,"\n\r",0);
             write_to_buffer(d,"Weight is limited from 50 to 1000.\n\r",0);
             break;

        }

        ch->pcdata->weight = weight;
	write_to_buffer(d,"\n\r",0);
